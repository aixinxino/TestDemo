...
const staticRoutes: AppRouteRecordRaw[] = [
    ...
    children: [
      {
        ...
        meta: { title: 'menus.dashboard.console', keepAlive: false, noLogin: true }
      }
    ]
  },
  {
  ...
  {
    path: RoutesAlias.DrawingBoard,
    name: 'DrawingBoard',
    component: () => import('@views/drawing_board/index.vue'),
    meta: { title: 'menus.drawingBoard.title', isHideTab: true }
  }
]
...
router.beforeEach(async (to, from, next) => {
  ...
  // 检查登录状态，如果未登录且要访问需要登录的页面则跳转到登录页
  ...
  // 如果用户已登录且动态路由未注册，且不是访问noLogin页面，则注册动态路由
